---
title: "Getting Started with the Spring Community REST API"
...
<Steps>
  ...
  <Step title="Start the application">
    **Option A — Docker Compose (recommended)**
    ...
    Docker Compose starts both the Spring Boot application and a PostgreSQL 15 database in one command. The `app` container expects the datasource password via `SPRING_DATASOURCE_PASSWORD`, and the `postgres` container expects it via `DB_PASSWORD`:
