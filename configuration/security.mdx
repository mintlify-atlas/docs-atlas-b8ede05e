...
description: "Spring Community uses Spring Security with a stateless JWT filter chain. Learn how to configure CORS origins, public routes, and session management for production."
---
...
This means every authenticated user carries the authority `ROLE_USUARIO`. The authority is included in the JWT claims and re-read by `JwtAuthFilter` on each request to reconstruct the `Authentication` object without a database round-trip.
