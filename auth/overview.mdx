---
title: "Authentication in Spring Community"
sidebarTitle: "Overview"
description: "Spring Community uses stateless JWT authentication with access and refresh tokens. Learn how to register, log in, and pass tokens to protected endpoints."
---
...
<Note>
  Although the `/ws-chat` WebSocket handshake URL itself is marked public in Spring Security, the STOMP `CONNECT` frame is intercepted by `JwtChannelInterceptor`, which validates the ACCESS token before the connection is established. An invalid or missing token will result in no principal being set on the session.
</Note>
